...
namespace Actudent\Admin\Controllers;
...
class Absensi extends \Actudent
{
	...
	public function save($scheduleID, $date, $includeHomework)
	{
		if (valid_token()) {
			...
			if (!$this->validateForms($data, $validation[0], $validation[1])) {
				...
				$data = $this->formData();
				$saved = null;
				$todayJournal = $this->absensi->getJournalByDate($date, $data['grade']);
				if ($includeHomework) {
				...
				if ($data['copyPresence'] === '1') {
					$presence = [];
					foreach ($todayJournal as $j) {
						// find presence only for other journal, NOT the journal currently saved
						if ($j->journal_id !== $saved->journal_id) {
							$findPresence = $this->absensi->getPresenceByJournal($j->journal_id);
							if (count($findPresence) > 0) {
	...
	private function formData()
	{
		return [
			...
			'copyPresence'          => $this->request->getPost('copyPresence')
		];
	}
