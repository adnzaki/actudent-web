...
namespace Actudent\Sync\Controllers;
		...
		if (is_admin()) {
			...
			if ($imporPtk === 'yes') {
				...
				foreach ($decoded as $d) {
					...
					if (!$this->model->pegawaiExists($staffNik, $d->ptk_id)) {
						$values = [
							'staff_nik'             => $staffNik,
							'staff_name'            => $d->nama,
							'staff_phone'           => null,
		...
		if (is_admin()) {
			...
			if ($option !== 'pdBaru') {
				...
				$firstGrade = array_filter($decoded, function ($item) use ($level) {
					if(isset($item->tingkat_pendidikan_id)) {
	...
	private function pushPesertaDidik($data)
	{
		...
		foreach ($data as $d) {
			...
			$parentValues = [
				...
				'user_name'             => $userName,
				'user_email'            => $userEmail . '@' . get_subdomain(),
			...
			if (!$this->model->pesertaDidikExists($d->nipd, $d->nama)) {
				...
				$rombelId = isset($d->rombongan_belajar_id) ? $d->rombongan_belajar_id : null;
