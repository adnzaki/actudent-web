		...
		<div class="row">
			<div class="col-12 col-md-4 col-lg-4 col-xl-3">
				<div class="form-group">
					<button type="button" class="btn btn-outline-info" data-toggle="modal" data-target="#tambahPegawaiModal"
						@click="showAddPegawaiForm">{+ lang Admin.tambah +}
					</button>
				...
				<tr v-for="(item, index) in data" :key="index" class="soft-dark">
					...
					<td>
						<button type="button" class="btn btn-icon btn-info mr-1" data-toggle="tooltip" data-placement="top"
							title="{+ lang Admin.perbarui +}" @click="getDetailPegawai(item.user_id)">
							<i class="la la-pencil"></i>
						</button>
						<button type="button" class="btn btn-icon btn-danger mr-1" data-toggle="tooltip" data-placement="top"
							title="{+ lang Admin.hapus +}" @click="singleDeleteConfirm(item.staff_id, item.user_id)">
