...
Karena penggunaan custom database group di versi terbaru ini, penggunaan method turunan `$this->validate()` digantikan oleh helper `validate()`. Cara penggunaan helper ini sama persis dengan `$this->validate()` bawaan CodeIgniter, hanya saja di belakang layarnya sudah menggunakan custom DB Group terbaru dari Actudent.
