<template>
  ...
  >
    <q-btn fab icon="add" :color="fabColor" @click="store.showForm = true" />
  </q-page-sticky>
</template>
...
import { conf, fabColor } from 'src/composables/common'
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const navDate = () => {
        ...
        const nextStartDate = formatDate(
          date.subtractFromDate(startDate, { days: 7 })
        )
        const nextEndDate = formatDate(
          date.addToDate(startDate, { days: 14, months: 1 })
        )
        store.calendar.view = initialView.value
