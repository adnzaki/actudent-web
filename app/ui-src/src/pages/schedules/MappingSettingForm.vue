<template>
  <q-dialog no-backdrop-dismiss v-model="store.showSettingsForm"
      ...
      <q-card-section class="scroll card-section">
        <q-form>
          <q-input outlined :label="`${$t('jadwal_label_alokasi')} (${$t('jadwal_menit')})`" 
            dense v-model="store.schedule.allocation"
            :rules="[val => val.match(/[^0-9]/) === null && val !== '' || $t('jadwal_alokasi_natural')]" 
            class="q-mb-xs"
          />
          <error :label="error.lesson_hour" />

          <q-input outlined v-model="store.schedule.startTime" mask="time" :rules="['time']">
