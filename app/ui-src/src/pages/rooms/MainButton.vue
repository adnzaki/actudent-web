  ...
  <div class="col-12 col-md-4">
    <div class="q-gutter-xs mobile-hide">
      <q-btn color="deep-purple" icon="add" class="q-pl-sm" :label="$t('tambah')"
        @click="$store.state.rooms.showAddForm = true" />
    ...
    <q-page-sticky position="bottom-right" 
      ...
      v-if="!$store.state.rooms.showAddForm">
...
export default {
  name: 'MainButton',
  methods: {
    ...mapMutations('rooms', ['multipleDeleteConfirm'])
  },
  setup() {
    const store = useStore()
    return {
      selected: computed(() => store.state.rooms.selectedRooms),
      fabPos
    }
  }
}
