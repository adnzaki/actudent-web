      ...
      <q-markup-table bordered>
        <thead>
          <tr>
            ...
            <th class="text-left cursor-pointer mobile-hide" @click="paging.sortData('room_code')">{{ $t('ruang_kode') }} <sort-icon /></th>
            <th class="text-left cursor-pointer" @click="paging.sortData('room_name')">{{ $t('ruang_nama') }} <sort-icon /></th>
            <th class="text-left">{{ $t('aksi') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in data" :key="index">
            <td :class="['text-left', checkColWidth()]"><q-checkbox v-model="store.selectedRooms" :val="item.room_id" /></td>
            <td class="text-left mobile-hide">{{ item.room_code }}</td>
            <td class="text-left">{{ item.room_name }}</td>
            <td class="text-left">
              <q-btn-group class="mobile-hide">
                <q-btn color="accent" icon="edit" @click="store.getDetail(item.room_id)" />
              ...
              <q-btn round icon="more_vert" color="accent" class="mobile-only" outline>
                <q-menu>
                  <q-list style="min-width: 100px">
                    <q-item clickable v-close-popup @click="store.getDetail(item.room_id)">
