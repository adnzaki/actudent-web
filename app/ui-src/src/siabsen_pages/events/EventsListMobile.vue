<template>
  <q-list class="q-px-md q-py-sm">
    <q-expansion-item
      ...
      default-opened
      v-for="(item, index) in userEvents" :key="index">
      <q-card>
        <q-card-section class="q-px-none q-pt-md q-pb-xs cursor-pointer"
          @click="$store.dispatch('agenda/getDetail', item.id)">
          <q-chip square style="font-size: 12px;top: 0; right: -7px" class="absolute"
            size="sm" :color="badgeColor(item.priority)" text-color="white">
        ...
        <q-card-actions align="center">
          <q-btn class="stafflist-btn" :label="$t('siabsen_do_absen')" 
            @click="openPresenceDialog(item.id, item.canPresent)" color="accent" />
        </q-card-actions>
      </q-card>
    </q-expansion-item>    
  </q-list>
...
export default {
  setup() {
    ...
    return {
      ...
      badgeColor: priority => {
        const colors = {
          'high': 'negative', 'normal': 'positive', 'low': 'blue'
        }
