<template>
  <div>
    ...
    <q-markup-table :separator="separator" flat bordered v-if="!store.spinner">
      <thead>
        <tr>
          ...
          <th class="text-center mobile-only" rowspan="2">
            {{ $t('staff_nama') }} / {{ $t('absensi_ringkasan') }}
          </th>
          <th class="text-center mobile-hide" colspan="3">
            {{ $t('absensi_ringkasan') }}
          </th>
      ...
      <tbody>
        <tr v-for="(item, index) in summary" :key="index">
          ...
          <td class="text-left mobile-only">
            {{ item.name }} <br />
            {{ $t('absensi_hadir') }}: <strong>{{ item.present }}</strong>
            <br />
